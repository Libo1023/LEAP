...
class ConvTransE(torch.nn.Module):
    def __init__(self, num_entities, embedding_dim, input_dropout=0, hidden_dropout=0, feature_map_dropout=0, channels=50, kernel_size=3, use_bias=True):
        ...
        self.fc_llm_1 = torch.nn.Linear(50265, 768)
        self.fc_llm_2 = torch.nn.Linear(embedding_dim+768, embedding_dim)
    
    def forward(self, 
                embedding, 
                emb_rel, 
                triplets, 
                sentence_embeddings                                  # New Sentence Embeddings
               ) : 
        ...
        x_text = self.fc_llm_1(sentence_embeddings)                  # [num_triplets, 256]
        x_text = F.relu(x_text)
        x = torch.cat((x, x_text), dim = 1)                          # [num_triplets, h_dim+256]
        x = self.fc_llm_2(x)                                         # [num_triplets, h_dim]
        ##################################################################################################
